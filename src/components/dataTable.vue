  ...
  <div class="cfj_table">
          ...
          >
            <template slot-scope="scope">
              ...
              >
                <img
                  style="width: 100%"
                  :src="tit.callback?tit.callback(scope.row[tit.field],scope.row):scope.row[tit.field]"
                  alt
                />
              </span>
            </template>
    ...
    <div v-if="ispagination" class="cfj_pagination">
      <el-pagination
        ...
        background
        :current-page="ispagination && ajax.data ? ajax.data.pageNumber:''"
  ...
  methods: {
    ...
    numerical (i) {
      if (this.ajax.data && this.ajax.data.pageNumber) {
        return (
          (this.ajax.data.pageNumber - 1) * this.ajax.data.pageSize + i + 1
        )
      } else {
        return ++i
      }
    ...
    getList () {
      ...
      this.$axios
        ...
        .then(res => {
          let data = res.data
          if (
            (data.total > 9) & (data.pageNumber > 1) &&
            data.list.length === 0
          ) {
